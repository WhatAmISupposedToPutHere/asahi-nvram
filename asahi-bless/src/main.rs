...
use nvram::{Variable, erase_if_needed, Nvram};
...
impl OmapVal<'_> {
    fn flags(&self) -> u32 {
        u32::from_le_bytes(self.0[0..0+4].try_into().unwrap())
    }
    fn size(&self) -> u32 {
...
impl BTreeNodePhys<'_> {
    ...
    fn table_space<'a>(&'a self) -> NLoc<'a> {
        NLoc(&self.0[40..])
    }
    fn nkeys(&self) -> u32 {
        u32::from_le_bytes(self.0[36..36+4].try_into().unwrap())
...
fn lookup(disk: &mut File, cur_node: &BTreeNodePhys, key: u64) -> Option<u64> {
    if cur_node.level() != 0 {
        unimplemented!();
    }
    if cur_node.flags() & BTreeNodePhys::FIXED_KV_SIZE != 0 {
        ...
        let val_end = cur_node.0.len() - if cur_node.flags() & BTreeNodePhys::ROOT == 0 { 0 } else { BTreeInfo::SIZE };
...
fn scan_volume(disk: &mut File) -> Result<HashMap<Uuid, Vec<String>>> {
    ...
    for i in 0..NxSuperblock::MAX_FILE_SYSTEMS {
        ...
        if let Ok(name) = std::str::from_utf8(trim_zeroes(asb.volname())) {
            vgs_found.entry(asb.volume_group_id()).or_default().push(name.to_owned());
        }
    }
    Ok(vgs_found)
}
...
fn swap_uuid(u: &Uuid) -> Uuid {
    ...
    return Uuid::from_fields(a.swap_bytes(), b.swap_bytes(), c.swap_bytes(), d).unwrap();
}
...
fn main() {
    ...
    let boot_str = format!("EF57347C-0000-AA11-AA11-00306543ECAC:{}:{}",
                           cands[ix].part_uuid.to_hyphenated().encode_upper(&mut Uuid::encode_buffer()),
                           cands[ix].vg_uuid.to_hyphenated().encode_upper(&mut Uuid::encode_buffer())
    );
    let mut file = OpenOptions::new().read(true).write(true)
        .open("/dev/mtd0").unwrap();
    let mut data = Vec::new();
    file.read_to_end(&mut data).unwrap();
    let mut nv = Nvram::parse(&data).unwrap();
    nv.prepare_for_write();
    nv.active_part_mut().system.values.insert(b"boot-volume", Variable {
        key: b"boot-volume",
